...
const tabConfig = {
  top: {
    title: 'Top',
    description: 'Классический список по рыночной капитализации: от первого места к последнему.',
...
const tabDefaultSort = {
  top: { key: 'market_cap', direction: 'desc' },
  trending: { key: 'price_change_percentage_24h', direction: 'desc' },
  watchlist: { key: 'total_volume', direction: 'desc' },
  prediction: { key: 'price_change_percentage_24h', direction: 'desc' },
  visited: { key: 'market_cap_rank', direction: 'asc' }
};

let allCoins = [];
let currentTab = 'top';
let sortState = { key: 'market_cap', direction: 'desc' };
...
tabButtons.forEach((button) => {
  button.addEventListener('click', () => {
    ...
    sortState = { ...tabDefaultSort[currentTab] };
    searchInput.value = '';
